  ...
  const newLines = lines.map(line => {
    ...
    if (importMatch) {
      ...
      if (!path.endsWith('.js') && !path.endsWith('.ts') && !path.endsWith('.json') && !path.includes('*')) {
        fixed++;
        // Fix known directory imports
        let newPath = path;
        if (path === '../../drizzle') {
          newPath = '../../drizzle/index.js';
        } else if (path === './schema') {
          newPath = './schema/index.js';
        } else if (path === './common/logger') {
          newPath = './common/logger/index.js';
        } else if (path === '../drizzle') {
          newPath = '../drizzle/index.js';
        } else if (path === '../../drizzle') {
          newPath = '../../drizzle/index.js';
        } else if (path === '../../../drizzle') {
          ...
          // Files like './something' -> './something.js'
          newPath = path + '.js';
        } else if (path.startsWith('../') && !path.includes('/', 3)) {
          // Files like '../something' -> '../something.js'
          newPath = path + '.js';
        } else {
          // Default: add .js
